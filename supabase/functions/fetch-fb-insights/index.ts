  ...
  try {
    ...
    if (campaignInsights.length > 0) {
      const campaignRows = campaignInsights.map((item: any) => {
        ...
        const revenue = conversions * (spend > 0 && cpa > 0 ? spend / cpa * (conversions || 1) : 0);
        // Simple ROAS: if we have purchase value actions
        ...
        return {
          ...
          status: 'active', // campaigns returned by insights are active in the period
          date_start: item.date_start,
          spend,
          impressions: parseInt(item.impressions) || 0,
          clicks: parseInt(item.clicks) || 0,
    ...
    // 3. Also fetch campaign statuses
    const campaignsUrl = `https://graph.facebook.com/v19.0/${accountId}/campaigns?fields=id,name,status,objective&access_token=${FACEBOOK_ACCESS_TOKEN}&limit=500`;
    const campaignsResponse = await fetch(campaignsUrl);
    const campaignsData = await campaignsResponse.json();

    if (!campaignsData.error && campaignsData.data) {
      // Update statuses for campaigns we already have
